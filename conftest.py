...
def get_firefox_driver():
    ...
    # 1. Сначала пробуем стандартный запуск Selenium (как у ревьюера)
    try:
        return webdriver.Firefox(options=options)
    except Exception:
        pass

    # 2. Если упало из-за бага Windows/Python 3.14 — ищем локальный geckodriver в кэше
    ...
    if matches:
        service = Service(executable_path=matches[-1])
        return webdriver.Firefox(service=service, options=options)
